...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
BoxWithNMSLimitCPUKernel(
    const torch::Tensor& tscores_,
    const torch::Tensor& tboxes_,
    const torch::Tensor& tbatch_splits_,
    double score_thres_,
    double nms_thres_,
  ...
  int num_boxes_classes = get_box_cls_index(
                              num_classes - 1,
  ...
  int batch_size = 1;
  std::vector<float> batch_splits_default(1, tscores.size(0));
  ...
  // outputs 1,2,3
  torch::Tensor out_scores = torch::zeros({0}, torch::dtype(torch::kFloat32));
  torch::Tensor out_boxes =
      torch::zeros({0, box_dim}, torch::dtype(torch::kFloat32));
  torch::Tensor out_classes = torch::zeros({0}, torch::dtype(torch::kFloat32));

  // outputs 5,6
  torch::Tensor out_keeps = torch::zeros({0}, torch::dtype(torch::kInt));
  torch::Tensor out_keeps_size =
      torch::zeros({batch_size, num_classes}, torch::dtype(torch::kInt));
  ...
  for (int b = 0; b < batch_splits.size(); ++b) {
    ...
    for (int j = 1; j < num_classes; j++) {
      auto cur_scores =
          scores.col(get_score_cls_index(j, input_boxes_include_bg_cls_));
      auto inds = utils::GetArrayIndices(cur_scores > score_thres_);
      auto cur_boxes = boxes.block(
          0,
          get_box_cls_index(
              j, cls_agnostic_bbox_reg_, input_boxes_include_bg_cls_) *
              box_dim,
          boxes.rows(),
          box_dim);

      if (soft_nms_enabled_) {
        auto cur_soft_nms_scores = soft_nms_scores.col(
            get_score_cls_index(j, input_boxes_include_bg_cls_));
    ...
    if (detections_per_im_ > 0 && total_keep_count > detections_per_im_) {
      ...
      auto get_all_scores_sorted = [&]() {
        ...
        std::vector<KeepIndex> ret(total_keep_count);
        ...
        std::sort(
            ret.data(),
            ret.data() + ret.size(),
            [&input_boxes_include_bg_cls_, &scores](
                const KeepIndex& lhs, const KeepIndex& rhs) {
              return scores(
                         lhs.second,
                         get_score_cls_index(
                             lhs.first, input_boxes_include_bg_cls_)) >
                  scores(rhs.second,
                         get_score_cls_index(
                             rhs.first, input_boxes_include_bg_cls_));
            });
    ...
    out_boxes.resize_(
        {out_boxes.size(0) + total_keep_count, out_boxes.size(1)});
    ...
    for (int j = 1; j < num_classes; j++) {
      ...
      Eigen::Map<EArrXf> cur_out_scores(
          out_scores.data_ptr<float>() + cur_start_idx + cur_out_idx,
          cur_keep.size());
      Eigen::Map<ERArrXXf> cur_out_boxes(
          out_boxes.data_ptr<float>() + (cur_start_idx + cur_out_idx) * box_dim,
          cur_keep.size(),
          box_dim);
      Eigen::Map<EArrXf> cur_out_classes(
          out_classes.data_ptr<float>() + cur_start_idx + cur_out_idx,
          cur_keep.size());
    ...
    Eigen::Map<EArrXi> out_keeps_arr(
        out_keeps.data_ptr<int>() + cur_start_idx, total_keep_count);
    Eigen::Map<EArrXi> cur_out_keeps_size(
        out_keeps_size.data_ptr<int>() + b * num_classes, num_classes);
  ...
  torch::Tensor batch_splits_out =
      torch::zeros({batch_size}, torch::dtype(torch::kFloat32));
  Eigen::Map<EArrXf> batch_splits_out_map(
      batch_splits_out.data_ptr<float>(), batch_size);
